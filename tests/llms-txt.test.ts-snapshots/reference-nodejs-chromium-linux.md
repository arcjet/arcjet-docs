...
In production, make the application reachable only through a proxy that overwrites or safely appends forwarding headers, and list every trusted hop in `proxies`. Use a proxy-service helper such as `cloudflare()` where available. Invalid proxy entries and non-empty malformed manual `ipSrc` values are rejected; an empty `ipSrc` is treated as omitted. Trust-all ranges produce a warning. Inspect the selection with `client.clientIpDetails(request)` in `@arcjet/node`, or `findIpDetails()` / `resolveClientIp()` from `@arcjet/ip`. Never copy an untrusted forwarding header into `ipSrc`.
...
The Arcjet Node.js SDK uses several environment variables to configure its behavior. For more information, see [Concepts: Environment variables](/environment). The `ARCJET_KEY` environment variable is not read automatically and must be passed explicitly.
