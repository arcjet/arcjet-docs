 [![npm badge](https://img.shields.io/npm/v/@arcjet/sveltekit?style=flat-square&label=%E2%9C%A6Aj&labelColor=ECE6F0&color=ECE6F0)](https://www.npmjs.com/package/@arcjet/sveltekit)
...
Create a new `Arcjet` object with your API key and rules. Put it in a server-only module in `/src/lib/server/`.
...
The Arcjet SvelteKit SDK uses several environment variables to configure its behavior. For more information, see [Concepts: Environment variables](/environment). The `ARCJET_KEY` environment variable is not read automatically and must be passed explicitly.
...
Arcjet supports the current major version of SvelteKit. When a new major version of SvelteKit is released, we bump the major version of the Arcjet SDK. The current supported major version is SvelteKit 2.x.x.
