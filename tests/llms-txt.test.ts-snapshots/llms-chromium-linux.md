...
- [CrewAI agent guard](https://docs.arcjet.com/guards/crewai): official CrewAI `guard_hooks` on `@on(PRE_TOOL_CALL)` plus `HookAborted`. `POST_TOOL_CALL` is capture only.
