...
- [Agent guard integrations](https://docs.arcjet.com/guards/framework-integrations): Vercel AI SDK, LangChain, LangChain JS, CrewAI, LangGraph, Genkit, OpenAI Agents, OpenAI Agents Python, Vercel Eve, Mastra, and Claude Agent SDK. One `ArcjetDenialResult` payload; per-framework envelopes.
...
- [OpenAI Agents Python agent guard](https://docs.arcjet.com/guards/openai-agents-py): inbound `guard()` before `Runner.run` and `guard_tool` on `FunctionTool.tool_input_guardrails` plus `reject_content`. Extra `arcjet[openai-agents]` is unpublished on `arcjet` 0.9.x.
