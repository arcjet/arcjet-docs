...
- **Global guard:** Applies Arcjet rules on every request, but doesn't let you
  configure rules per route.
- **Per route guard:** Lets you configure rules per route, but requires you to
  add the guard to every route and has limited flexibility.
...
Arcjet applies the rules and returns a generic error if the result is `DENY`.
