...
The following example shows how to configure a rate limit on a single API route.
It applies a limit of 60 requests per hour per IP address. If the limit is
...
The following example is the same as the preceding one. However this example
also shows a customized response rather than the default.
...
tokens is variable depending on the request, such as conversation length or image
size.
...
The following example configures a token bucket rate limit using the
...
<SelectableContent client:load syncKey="language">
  ...
  <div slot="TS (Pages)" slotIdx="2">
    The Next.js pages router does not support streaming responses, so use the
    app router for this example. You can still use the `pages/` directory for
    the rest of your application. For details, see the [Next.js AI
    docs](https://sdk.vercel.ai/docs/guides/frameworks/nextjs-pages).
  </div>
  <div slot="JS (App)" slotIdx="3">
...
can specify the characteristics Arcjet uses to track the limit.

The following example shows how to configure a rate limit on a single API route.
It applies a limit of 60 requests per hour per API key, where the key is
provided in a custom header called `x-api-key`. If the limit is exceeded, the
client is blocked for 10 minutes before being able to make any further requests.
...
Using Next.js middleware lets you set a rate limit that applies to every
route:
