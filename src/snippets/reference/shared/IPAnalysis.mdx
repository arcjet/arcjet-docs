...
The `ArcjetDecision` object contains an `ip`
...
check their availability. These methods also refine the type, which removes the
need for null or undefined checks.
...
satellite internet providers, and even ordinary users. Likewise with the
...
mislocated. These fields are provided for convenience, such as suggesting a user
location, but don't rely on them alone.
...
check their availability. This method also refines the type, which removes the
need for null-ish checks.
...
To deny VPNs, proxies, Tor, hosting IPs, or other high-risk networks using
these fields, see
...
- `isSafe` (`boolean`): whether the IP is trusted infrastructure rather than a
  threat.
...
method to check the availability. This method also refines the type, which
removes the need for null-ish checks.
...
- `service` (`string | undefined`): the name of the service associated with the
  IP address, such as `Apple Private Relay`.
