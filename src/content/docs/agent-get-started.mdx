...
description: "A step-by-step guide for AI coding agents to set up Arcjet security in any project. Connect via MCP, get your API key, install the SDK, and add protection."
prev: false
next: false
---
...
Arcjet is the runtime security layer for AI apps. It protects your application
from bots, prompt injection, PII leaks, and abuse — all configured in code,
not a separate WAF or proxy. This page walks AI coding agents through the full
setup flow.
...
coding tool. Connect it to retrieve your API key and verify requests.
...
See the <Link.Page href="/mcp-server">MCP server docs</Link.Page> for more
details and additional clients (ChatGPT, Claude Desktop).
...
[app.arcjet.com](https://app.arcjet.com) to create one (free).
...
These prompts work well when given to an AI coding agent with the Arcjet MCP
server connected:
