---
title: "Architecture"
description: "How Arcjet works: a local SDK that makes most decisions in process, a WebAssembly analysis engine, and a regional Cloud API for the decisions that need shared state."
---
...
- Proper security protections need the full context of the application, which is
  why security rules and protections belong alongside the code they are
  protecting.
- Security rules must be easy to test. You must be able to run them in
  development as well as production.
- Arcjet must not interfere with the rest of the application. It must be easy to
  install, must not add significant latency to requests, and must not require
  changes to the application's architecture.

## Why in-application enforcement matters

Arcjet is designed to run behind existing WAFs, CDNs, API gateways, and bot
defenses, adding the business context those layers cannot reliably infer. It can
evaluate identity, session, plan, route, request body, and application workflow
state before your code completes a protected action.

## Policy ownership and remote rules

Arcjet separates where enforcement runs from who operates policy. Developers add
the SDK at the application boundary and keep app-specific rules close to the
routes, tools, and workflows they protect. Security and operations teams can use
<Link.Page href="/remote-rules">remote rules</Link.Page> to adjust site-wide
rules from the Arcjet dashboard or
<Link.Page href="/mcp-server">MCP server</Link.Page> without waiting for a code
change.

Both paths feed the same decision model. Code-defined rules carry application
context and reviewability; remote rules provide an operational control plane for
incident response, dry-run tuning, and temporary mitigations. The SDK returns one
decision to your application so enforcement still happens in your runtime.
...
- **<Link.Page href="/shield">Shield</Link.Page>** – Protect against common attacks.
- **<Link.Page href="/rate-limiting">Rate limiting</Link.Page>** – Limit the number of requests
  from a client over a period of time.
- **<Link.Page href="/bot-protection">Bot protection</Link.Page>** – Detect and block automated
  clients.
- **<Link.Page href="/email-validation">Email validation</Link.Page>** – Check
  that an email address is valid.
- **<Link.Page href="/sensitive-info">Sensitive information</Link.Page>** – Protect against
  clients sending you PII that you do not wish to handle.
- **<Link.Page href="/filters">Filters</Link.Page>** – Block requests using expressions over
  request metadata.
...
and decide how to handle it. This could be as straightforward as blocking the
request or more advanced like flagging an account for review.

Rules can also be configured outside of your application as
<Link.Page href="/remote-rules">remote rules</Link.Page>, managed from the
Arcjet dashboard or through the
<Link.Page href="/mcp-server">Arcjet MCP server</Link.Page>. Remote rules are
evaluated by the Cloud API and combined with SDK rule results into a single
decision, so SecOps can change protection behavior for a site without editing
code or redeploying while developers keep application-specific enforcement in
the code path.
...
3. Your server, such as a container, serverless function, or VM, receives the
   request.
...
4. The request is blocked and the client receives an error message, but your
   ...
   request, it may have been able to determine that it was legitimate, for
   example because the client was logged in.
...
no additional infrastructure. The Arcjet SDK calls the Cloud API to get the
...
similar solutions. Arcjet also attempts to cache decisions locally using an
in-memory cache. In many cases, the total latency is less than 1&nbsp;ms.

#### Example: Rate limit rule (allow)

1. A client makes an HTTP request to your application. It passes through the
   network level DDoS protection as described earlier then reaches your server.
...
This means that if the same client makes another request, the SDK reads from the
local cache and returns the response immediately:
...
- A <Link.Page href="/shield">Shield</Link.Page> rule is configured. Behavior must be compared
  across multiple requests to protect against common attacks.
- A <Link.Page href="/rate-limiting">Rate limiting</Link.Page> rule is configured. Client state must
  ...
  services.
- A <Link.Page href="/bot-protection">Bot protection</Link.Page> rule is configured and local
...
- An <Link.Page href="/email-validation">Email validation</Link.Page> rule is
  configured and the email address syntax is valid. The Arcjet SDK analyzes the
  email address locally to check the syntax. If it is invalid, the SDK returns a
  decision immediately. If it is valid, the SDK calls the Cloud API to verify
  the email address: does it have a valid MX record, and is it a disposable
  email address?
- A <Link.Page href="/filters">Filter</Link.Page> rule is configured that inspects `ip.src.*` IP
...
minimize latency for each request. This adds less than 1&nbsp;ms of overhead to a
typical request.

When a call to the Cloud API is required there is some additional latency before
a decision is made. The Cloud API has been designed for high performance and low
latency, and is deployed to multiple regions around the world. The SDK
automatically uses the closest region, which means the total overhead is
typically no more than 20&nbsp;ms to 30&nbsp;ms, often significantly less.

Arcjet only ever makes a single call to the API regardless of the number of
rules configured.

Where a decision has been cached locally, such as blocking a client, Arcjet
continues to function even if the service is unavailable. In the event of a
network problem or outage and a call to the API is needed, the Arcjet SDK
defaults to failing open, so it allows all requests. This is a deliberate design
decision to ensure that your application continues to function even if there is
a problem, but you can configure the SDK to fail closed if you prefer. See an
example of how to handle errors and fail open or closed in the <Link.Page
href="/reference/nodejs#error-handling">Node.js SDK reference</Link.Page>.

#### Timeout

The total overhead when connecting to the Arcjet Cloud API is typically no more
than 20&nbsp;ms to 30&nbsp;ms,
...
Conversely,
a longer timeout may improve success rates but could lead to a less responsive
experience.

For more info,
see the post [Minimizing latency for humans &
machines](https://blog.arcjet.com/minimizing-latency-for-humans-machines/)
on the Arcjet blog.
...
client reaches a threshold of suspicious activity, Arcjet issues a deny decision
for that client for a period of time.
...
In the following example, the results of the post-request analysis from previous
...
5. The client makes another request and the result of the post-request analysis
   is used to inform the decision. Arcjet applies this if you have configured a
   <Link.Page href="/shield">Shield</Link.Page> rule.
