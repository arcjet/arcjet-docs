...
## Why does in-application enforcement matter vs a CDN WAF?

Arcjet runs inside your application so it can use identity, session, plan, and
request-body context a CDN WAF cannot reliably infer. Enforcement happens in
your runtime, so a logged-in user or workflow state can change the decision
...
## When does Arcjet call the Cloud API vs decide locally?

Arcjet decides locally when request headers, email syntax, or a cached deny are
enough, and it calls the Cloud API when a rule needs shared state or reputation
data. Rate limiting, Shield, live bot checks, email verification, and
`ip.src.*` filters require the Cloud API.
...
- An <Link.Page href="/email-validation">Email validation</Link.Page> rule is configured and the
...
## What is Arcjet Cloud API latency?

Arcjet Cloud API overhead is typically 20–30ms when a remote decision is
required, and published p95 is around 25ms. Local analysis and cached decisions
often add less than 1ms.
...
typically no more than 20-30ms, often significantly less. See
[How we achieve our 25ms p95 response time SLA](https://blog.arcjet.com/how-we-achieve-our-25ms-p95-response-time-sla/)
on the Arcjet blog.
...
The default is 500ms in production and 1000ms in development.
...
Arcjet fail behavior depends on the API: HTTP `protect()` and direct `guard()`
fail open by default, while Vercel AI SDK and LangChain wrappers fail closed.
The Cloud API default timeout is 500ms in production; see
<Link.Page href="/architecture#how-arcjet-fails-when-a-security-check-cannot-finish">How Arcjet fails when a security check cannot finish</Link.Page>
for the per-API defaults and how to choose.

## How Arcjet fails when a security check cannot finish

Arcjet does not apply one global fail mode when a security check cannot finish;
each API and wrapper documents its own default. Direct `guard()` fails open: it
returns ALLOW plus error codes rather than treating an incomplete check as a
policy denial, and `hasFailedOpen()` (or `has_failed_open()` in Python)
identifies that case. `protect()` on HTTP routes fails open if the Cloud API is
unreachable, so a network problem does not block all requests; that default is
configurable (see the
<Link.Page href="/reference/nodejs#error-handling">Node.js SDK reference</Link.Page>).
Vercel AI SDK and LangChain wrappers fail closed unless you set
`onGuardError: "allow"` / `on_guard_error="allow"`. A search tool and a refund
tool should not share one global fail mode; the caller chooses for their threat
model. Cloud API calls typically add 20–30ms, local analysis is often under
1ms, and the default timeout is 500ms in production and 1000ms in development.
