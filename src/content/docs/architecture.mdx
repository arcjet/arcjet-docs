...
- **[Shield](/shield/intro)** - Protect against common attacks.
- **[Rate limiting](/rate-limiting/intro)** - Limit the number of requests
  from a client over a period of time.
- **[Bot protection](/bot-protection/intro)** - Detect and block automated
  clients.
- **[Email validation](/email-validation/intro)** - Check
  that an email address is valid.
- **[Sensitive information](/sensitive-info/intro)** - Protect against
  clients sending you PII that you do not wish to handle.
- **[Filters](/filters/intro)** - Block requests using expressions over
  request metadata.
...
- A [Shield](/shield/intro) rule is configured. Behavior must be compared
  across multiple requests to protect against common attacks.
- A [Rate limiting](/rate-limiting/intro) rule is configured. Client state must
  ...
  services.
- A [Bot protection](/bot-protection/intro) rule is configured and local
...
- An [Email validation](/email-validation/intro) rule is configured and the
...
- A [Filter](/filters/intro) rule is configured that inspects `ip.src.*` IP
