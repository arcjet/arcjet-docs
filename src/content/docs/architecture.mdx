...
- **[Shield](/shield)** - Protect against common attacks.
- **[Rate limiting](/rate-limiting)** - Limit the number of requests
  from a client over a period of time.
- **[Bot protection](/bot-protection)** - Detect and block automated
  clients.
- **[Email validation](/email-validation)** - Check
  that an email address is valid.
- **[Sensitive information](/sensitive-info)** - Protect against
  clients sending you PII that you do not wish to handle.
- **[Filters](/filters)** - Block requests using expressions over
  request metadata.
...
- A [Shield](/shield) rule is configured. Behavior must be compared
  across multiple requests to protect against common attacks.
- A [Rate limiting](/rate-limiting) rule is configured. Client state must
  ...
  services.
- A [Bot protection](/bot-protection) rule is configured and local
...
- An [Email validation](/email-validation) rule is configured and the
...
- A [Filter](/filters) rule is configured that inspects `ip.src.*` IP
