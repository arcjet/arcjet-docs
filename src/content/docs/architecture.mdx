...
- **[Filters](/filters/concepts)** - Block requests using expressions over
  request metadata.
...
server close to the API.
In development your app may run on a laptop some distance away from the API.
That introduces additional latency.
To illustrate,
a developer in New Zealand may connect to the API in Australia,
whereas a user in New Zealand may connect to your app in Australia,
close to the API there.

A shorter timeout may result in faster failure responses but could lead to
increased error rates if the API needs time to respond.
Conversely,
