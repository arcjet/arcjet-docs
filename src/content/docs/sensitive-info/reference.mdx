...
# Built-in entity types (SensitiveInfoEntityType enum members):
...
For broader coverage, the `@arcjet/sensitive-info-rampart` package provides a
backend powered by the on-device
...
:::note
The `backend` option and the Rampart package are available in the JavaScript /
TypeScript SDKs only. The Python SDK always uses the built-in engine.
:::
...
model detects is a built-in `ArcjetSensitiveInfoType`, so you can list the types
directly in `allow` or `deny`:
...
:::note
The same backend also works with the `localDetectSensitiveInfo` rule in
<Link.Page href="/guards">Arcjet Guards</Link.Page> — pass `backend: rampart()`
there too to scan text inside agent tool calls and pipelines that don't route
through HTTP.
:::

The full set of types the backend supports is exported as `rampartEntities`,
which is convenient when you want to deny everything it can detect:
...
enough to run inline on each request:
...
:::note
The token-based [`detect`](#custom-entity-detection) callback of the
`sensitiveInfo` rule is **not** used by the Rampart backend, which works on
character spans rather than tokens. Add a `recognizer` instead to perform custom
detection.
:::
...
- It requires a server runtime with filesystem and native-addon access —
  Node.js, Bun, or Deno. It does **not** run on edge runtimes.
