...
government or financial identifiers in addition to the structured types.
...
examining the execution results, see [decision](#decision).
:::
...
locally so the rule can detect **names, addresses, and government or financial
...
Runtime](https://onnxruntime.ai/) and defaults to CPU execution; select a
...
- **Non-Latin scripts** (for example Chinese, Japanese, Korean, Arabic, Hindi,
  ...
  text.
- **Government or financial identifiers without a checksum** (passports,
  licenses, case numbers) are recognized less reliably than the structured types
...
- A server build must not bundle it, or the runtime fails to locate the model
  weights.
...
This example logs the full result as well as the sensitive info rule:
...
The entity types available to `allow`/`deny` depend on the active [detection
backend](#detection-backends). Every type in the following list is a built-in
