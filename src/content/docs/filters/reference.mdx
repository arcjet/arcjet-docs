...
- `allow` - A list of expressions that result in an `ALLOW` decision if the
  request matches. If none of the expressions match, Arcjet denies the request.
- `deny` - A list of expressions that result in a `DENY` decision if the request
  matches. If none of the expressions match, Arcjet allows the request.
...
For details of examining the execution results, see [decision](#decision).
:::
...
You can also split the expressions within the array for readability. Arcjet
combines them as an `OR` operation:
...
To combine multiple conditions, such as with `and`, use a single expression
string.
...
matching (beyond the per-field decoding described later). This is intentional:
...
- **Case matters.** `http.request.uri.path contains "login"` does not match a
  ...
  `upper()`, or use the <Link.Page
  href="/filters/reference#operators">`matches`</Link.Page> (`~`) operator with
  a case-insensitive regex flag, for example `http.request.uri.path ~
  "(?i)login"`.
...
typically no more than 20&nbsp;ms to 30&nbsp;ms, often significantly less. For
details, see the <Link.Page href="/architecture#arcjet-cloud-api">Architecture reference</Link.Page>.
...
To check whether a filter rule returned a deny conclusion, use
