...
The expression language is based on Wireshark display filters and looks for example like this:
...
Like any programming language, there are fields (`ip.src.vpn`),
functions (`lower()`), operators (`or`, `matches`), and values (`"curl"`).

Another example, this time to match IP addresses in the European Union:
...
All `ip.src.*` fields reflect information about `ip.src` which is the client IP
address.
