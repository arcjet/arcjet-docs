...
=======
<SlotByFramework>
>>>>>>> 3168ed0e (chore: add SDK route infrastructure and parity tests)
...
```py
from arcjet import Mode, arcjet, detect_bot, is_verified_bot
...
# Use the is_verified_bot() helper exported by arcjet:
if any(is_verified_bot(r) for r in decision.results):
    print("Verified bot")
    # Allow the request
...
Use the `is_missing_user_agent` helper from `arcjet`:

```py
from arcjet import is_missing_user_agent

if any(is_missing_user_agent(r) for r in decision.results):
    logger.warning("request missing required user-agent header")
    # You could return a 400 Bad request error here
