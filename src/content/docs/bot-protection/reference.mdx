  ...
  - text: "Configuration"
    ...
    children:
      - text: "Allowing specific bots"
        anchor: "allow-specific-bots"
      - text: "Denying specific bots"
        anchor: "deny-specific-bots"
...
and `deny` lists are mutually exclusive. With `allow`, Arcjet denies any
detected bot that the list does not name. With `deny`, Arcjet allows any
detected bot that the list does not name.
...
see [Decision](#decision). For the Python priority table, see
<Link.Page href="/reference/python#multiple-rules">Multiple rules in the Python SDK reference</Link.Page>.
...
bots](https://arcjet.com/bot-list), from <Link.Page href="/bot-protection/identifying-bots#bot-categories">bot categories</Link.Page>, or from both.
...
The <Link.Page href="/bot-protection/quick-start">quick start example</Link.Page> denies requests that match the bot detection rules, immediately
...
The decision also contains all of the <Link.Page href="/bot-protection/identifying-bots">identified bots and matched categories</Link.Page> detected from the request. A request
...
certain bots, any `deny` rules bypass verification and block bots matching the
`deny` list.
:::
...
The companies that operate these bots make them verifiable so application
developers can choose to avoid <Link.Page
href="/bot-protection/advanced-signals">additional signals</Link.Page> about the
request.

For example, when a request claims to be GoogleBot, Arcjet checks whether the IP
...
This checks whether the bot is spoofed. You would usually return a 403 or
similar response to block the request.
...
This checks whether the bot is verified.
