...
`allow` or `deny` options while writing your rules. In Python, the same string
identifiers are accepted on `allow` / `deny`; categories are also available via
the `BotCategory` enum.
...
- `CATEGORY:WEBHOOK`: Automated HTTP notifications for events e.g. Stripe payments
...
The Python SDK also accepts the underlying string identifiers (e.g.
...
you can use <Link.Page href="/filters">Arcjet Filters</Link.Page>.
See the <Link.Page href="/blueprints/malicious-traffic">Malicious traffic</Link.Page> blueprint for how to
block custom bots.
...
If you configure an `allow` rule and do not include `UNKNOWN_BOT`, then detected
bots that cannot be identified will be blocked. This is the default behavior to
