...
The `allow` and `deny` lists are mutually exclusive. With `allow`, Arcjet denies
any email type that the list does not name. With `deny`, Arcjet allows any email
type that the list does not name.
