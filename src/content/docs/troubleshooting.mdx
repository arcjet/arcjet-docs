...
Arcjet's <Link.Page href="/blueprints/ip-geolocation">IP geolocation</Link.Page>
and <Link.Page href="/blueprints/vpn-proxy-detection">VPN & proxy detection</Link.Page>
features require a real IP address to work. In local
development, Arcjet defaults to `127.0.0.1` so the IP analysis metadata will be
empty.
...
If you are seeing this error frequently, please
...
<Link.Page href="/guards/reference#how-arcjet-fails-when-a-security-check-cannot-finish">How Arcjet fails when a security check cannot finish</Link.Page>.
...
automatically detected IP is invalid (e.g. `127.0.0.1`), the request is rejected
to prevent spoofing.
...
determine whether it is running locally or not then it will be safe and assume
it is running in production. Automatic detection disallows private/internal
addresses in production, but allows them in development so you can test locally.
...
this error. Explicit application-provided IP values bypass automatic detection
and must be validated by your application.
...
Check out the <Link.Page href="/shield">Shield documentation</Link.Page> to
learn how to configure Shield rules. To restore the old behavior you can add a
simple Shield rule:
...
<Link.ToSdk href="/sensitive-info/reference#accessing-the-body" sdk="node">Accessing the body</Link.ToSdk>
...
Upgrade your Next.js version to the latest version to fix this error.
