...
const aj = arcjet({
  ...
  // Read more: <https://docs.arcjet.com/concepts/environment#arcjet-key>.
...
environment variable [`ARCJET_ENV`](/concepts/environment#arcjet-env) to
...
[`ARCJET_ENV`](/concepts/environment#arcjet-env) is not incorrectly set to
`development`.
...
variable [`ARCJET_ENV`](/concepts/environment#arcjet-env) to `development` in
your `.env.local` file:
