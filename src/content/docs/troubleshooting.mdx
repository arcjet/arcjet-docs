...
```py
# Use the `ip_src` keyword argument on `protect()` to override the client IP.
# You could read this from an environment variable to ensure it is only set
# in development.
# ip = "8.8.8.8"  # Google's public DNS server
decision = await aj.protect(request, ip_src=ip) if ip else await aj.protect(request)
```
...
`development`. This will disable the IP address validation and allow you to test
