...
field in the `request` object that is passed to the Arcjet `protect` function.

Which field you need to override depends on the SDK you are using. For example,
with Node.js you would override `socket.remoteAddress` instead:

```ts
const ip = process.env.DEV_IP ? process.env.DEV_IP : undefined;
const decision = await aj.protect({ ...req, socket: { remoteAddress: ip } });
```

In Next.js you would override the `ip` field:
