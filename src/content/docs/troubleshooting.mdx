...
environment variable [`ARCJET_ENV`](/environment#arcjet-env) to
`development`. This will disable the IP
...
variable [`ARCJET_ENV`](/environment#arcjet-env) to `development` in
your `.env.local` file:
...
Check out the [Shield documentation](/shield) to learn how to configure
