...
`rules=[...]` list. See [Custom verification logic](#custom-verification-logic)
...
<SlotByFramework client:load>
  <Fragment slot="python-fastapi">
    ...
    `validate_email`, and `sliding_window`. See [Custom verification
    logic](#custom-verification-logic) for the recommended Python pattern.
    :::
...
(JS) / `decision.is_denied()` (Python). To narrow down the reason to an email
validation rule, use `decision.reason.isEmail()` (JS) /
`decision.reason_v2.type == "EMAIL"` (Python).
