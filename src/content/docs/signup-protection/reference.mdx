...
primitives. The configuration options are the same, but specified in a single
rule.
...
The Python SDK does not include a `protect_signup` composite rule. The
equivalent configuration is to add `detect_bot`, `validate_email`, and
`sliding_window` rules independently to the `arcjet(...)` / `arcjet_sync(...)`
`rules=[...]` list. For a complete Python example, see [Custom verification
logic](#custom-verification-logic).
...
<SlotByFramework client:load>
  <Fragment slot="python-fastapi">
    ...
    you compose the three building blocks manually: `detect_bot`,
    `validate_email`, and `sliding_window`. For the recommended Python pattern,
    see [Custom verification logic](#custom-verification-logic).
    :::
...
<SlotByFramework client:load>
  <Fragment slot="python-fastapi">
    ...
    Because Python has no composite `protect_signup` rule, the equivalent
    recommended configuration manually composes `detect_bot`, `validate_email`,
    and `sliding_window`. See the [Custom verification
    logic](#custom-verification-logic) snippet for a complete Python example.
