...
The Arcjet Nuxt SDK works with Nuxt environment variable support.
You can use `.env` files which are automatically loaded in development.
See [“Environment variables” in the Nuxt
docs](https://nuxt.com/docs/4.x/guide/going-further/runtime-config#environment-variables)
for more info.

The Arcjet Nuxt SDK uses several environment variables to configure its behavior.
See [Concepts: Environment variables](/concepts/environment) for more info.
The `ARCJET_KEY` environment variable is not read automatically and must be passed
explicitly in `nuxt.config.ts`.
...
development (see [`ARCJET_ENV`](/concepts/environment#arcjet-env)) and 500ms
...
in development (see [`ARCJET_ENV`](/concepts/environment#arcjet-env)),
