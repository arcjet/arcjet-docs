...
description: "Reference for the Arcjet Nuxt SDK: install `@arcjet/nuxt`, configure the module, protect a server route or middleware, and read the decision."
prev: false
...
import CloudflareConfigTS from "@repo/nuxt/server/utils/CloudflareConfig.ts?raw";
import CloudflareConfigJS from "@repo/nuxt/server/utils/CloudflareConfig.js?raw";
...
[<NpmSdkVersion packageName="@arcjet/nuxt" />](https://www.npmjs.com/package/@arcjet/nuxt)
...
:::note
If you use Bun to run your Nuxt app,
...
This happens because Nuxt bundles for Node.js but Bun does not support all
Node.js APIs.
To solve this configure Nitro in Nuxt to bundle for Bun by adding the following
to your `nitro.config.ts`:
...
See the <Link.ToSdk href="/get-started" sdk="nuxt">quick start guide</Link.ToSdk>.

## Configuration

Arcjet is configured as an integration in your `nuxt.config.ts` file. Add your
Cloud API key as an environment variable and configure the rules you want to
apply.

### API key

First, get your site key from the [Arcjet dashboard](https://console.arcjet.com).
...
- `rules` - The rules to apply to the request. See the various sections of the
  ...
  <Link.ToSdk href="/shield/reference" sdk="bun">shield</Link.ToSdk>,
  <Link.ToSdk href="/rate-limiting/reference" sdk="bun">rate limiting</Link.ToSdk>,
  <Link.ToSdk href="/bot-protection/reference" sdk="bun">bot protection</Link.ToSdk>,
  <Link.ToSdk href="/email-validation/reference" sdk="bun">email validation</Link.ToSdk>.
...
The Arcjet Nuxt SDK uses several environment variables to configure its
...
is not read automatically and must be passed explicitly in `nuxt.config.ts`.
...
<Tabs>
  <TabItem label="TS">
    <Code
      code={removeTSCCommentDirectives(CloudflareConfigTS)}
      lang="ts"
      title="server/utils/arcjet.ts"
    />
  </TabItem>
  <TabItem label="JS">
    <Code
      code={removeTSCCommentDirectives(CloudflareConfigJS)}
      lang="js"
      title="server/utils/arcjet.js"
    />
  </TabItem>
...
For Nuxt, `requestInput` in the following example is the H3 event passed to the
server route, API route, or middleware handler.
...
- `ip` (`ArcjetIpDetails`) – An object containing Arcjet's analysis of the
  client IP address. For more information, see IP analysis.
...
{/* See the <Link.ToSdk href="/shield/reference" sdk="nuxt">shield documentation</Link.ToSdk> for more information */}
...
{/* See the <Link.ToSdk href="/rate-limiting/reference" sdk="nuxt">rate limiting documentation</Link.ToSdk> for more */}
...
{/* See the <Link.ToSdk href="/email-validation/reference" sdk="nuxt">email validation documentation</Link.ToSdk> for */}
...
a default one. You don\'t usually need to provide a client – the Arcjet Astro
