...
The Arcjet Deno SDK works with Deno environment variable support.
Deno can load `.env` automatically when running Deno with the `--env-file` flag.
See [“Environment variables” in the Deno
docs](https://docs.deno.com/runtime/reference/env_variables/) for more info.

The Arcjet Deno SDK uses several environment variables to configure its behavior.
