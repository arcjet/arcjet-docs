...
> **Caution:** The Go SDK is pre-release and its API may change. The current
> pre-release is `v1.0.0-rc.2`, which requires Go 1.25 or later.
...
Use Go tooling so the project records a compatible version in `go.mod`. Name the
pre-release explicitly, because `@latest` selects the newest stable tag and this
API is not in one yet:
