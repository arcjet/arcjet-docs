...
configured proxy hops. The selection is logged at debug level with the
`client_ip_provenance` facet. If an integration ever uses an unverified
forwarding header, Arcjet warns once per client.
...
rejected; trust-all ranges produce a warning. Use
...
`manual`, `development`, `request`, or `none`. Debug logs expose it as
