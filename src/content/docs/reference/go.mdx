...
The Go SDK requires Go 1.25 or later.
...
| `arcjet.GuardAction` | The same operations, when you want the helper to run the function only if policy allows it |
| `github.com/arcjet/arcjet-go/agentframework` | Microsoft Agent Framework tools and agent runs |
...
Guard labels are slugs: lowercase letters, digits, dashes, underscores, and
dots, starting and ending with a lowercase letter or digit. Uppercase is
rejected, so use `tools.get-weather` or `tools.get_weather`, not
`tools.getWeather`. Use a stable, hardcoded label for each operation and an
explicit rate-limit key such as a user ID, session ID, or API key.

`Bucket` on a rate-limit rule is a separate identifier from the Guard label.
It groups counters. For more information about rate limiting, see
<Link.Page href="/rate-limiting">Rate limiting</Link.Page>.

A label the service will not match reads as `ALLOW` with `HasFailedOpen()`
false, so the guard does not run. Check a label you build yourself with
`ValidateGuardLabel`. `Capture` warns `AJ1023` and still sends.
...
`Guard` returns a decision for you to act on. `GuardAction` runs a function only
if the policy allows it, so the decision and the effect cannot drift apart. It
is the fail-closed helper for consequential effects: tool calls, jobs, and
workers.
...
`Action` is required and is sent as the Guard label. Every call reaches Guard,
including one with no rules, because the server selects a remote policy by that
label. Set `Resolve` to compute `Actor`, `Inputs`, and `Rules` at call time; it
replaces the static fields.

A `DENY` decision returns `*GuardDeniedError` and does not run the function,
whatever `OnGuardError` is set to. Policy that could not be evaluated, because
Guard returned an error, the decision failed open, or `Resolve` failed, returns
`*GuardUnavailableError` under the default `OnGuardErrorDeny`, and runs the
function under `OnGuardErrorAllow`.

Each call records one capture event named by `Action` whose metadata `outcome`
is `success`, `degraded`, `denied`, `error`, or `unavailable`. `GuardAction`
bounds its own Guard call at two seconds when the caller's context carries no
deadline.

For Microsoft Agent Framework tools, the
<Link.Page href="/guards/microsoft-agent-framework">agent framework helpers</Link.Page>
place the same check around a tool and hand the model a denial it can read
instead of returning an error.

### Correlate decisions

A correlation ID groups the decisions of one run into a single Sequence in the
Arcjet Console. `GuardAction` and the agent framework helpers read it from the
context:

```go
ctx = arcjet.ContextWithCorrelationID(ctx, conversationID)
id, ok := arcjet.CorrelationIDFromContext(ctx)
```

`Guard` and `Capture` never read the context; pass them `CorrelationID`
explicitly. An empty ID is ignored rather than stored, so a call that passes
through a missing value does not shadow an ID an outer layer already set.
Nothing here generates an ID.
