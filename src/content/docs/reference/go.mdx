...
The Go SDK is `v1.0.0` and requires Go 1.25 or later. The Microsoft Agent
Framework helpers live in `github.com/arcjet/arcjet-go/agentframework` and
...
For Microsoft Agent Framework for Go, also install the adapter module:
...
### Guard a function with `GuardAction`

For a sensitive tool call or other Go function, wrap it in
`arcjet.GuardAction` instead of hand-writing the `HasFailedOpen()` gate. It
calls Guard even when you submit no rules, denies with
`*arcjet.GuardDeniedError`, fails closed with
`*arcjet.GuardUnavailableError` when policy cannot be evaluated, runs the
function otherwise, and records one capture event whose metadata `outcome`
is `success`, `degraded`, `denied`, `error`, or `unavailable`.
...
Distinguish the two errors with `errors.As`.
`OnGuardError: arcjet.OnGuardErrorAllow` opts a call site back into
fail-open; a `DENY` still blocks. Use `arcjet.NewGuardDenialResult(decision)`
and `arcjet.NewGuardUnavailableResult()` when the caller is a model and
needs a JSON result rather than a Go error.

For Microsoft Agent Framework for Go tools, use
<Link.Page href="/guards/agent-framework-go">`GuardTool`</Link.Page>
instead of wrapping each handler by hand.
