...
The Arcjet SvelteKit SDK works with SvelteKit environment variable support.
It reads from
[`$env/dynamic/private`](https://svelte.dev/docs/kit/$env-dynamic-private).
Whether SvelteKit reads `.env` files depends on the adapter.
When using `adapter-node`, SvelteKit loads `.env` automatically in development.
See [“Environment variables” in the SvelteKit `adapter-node`
docs](https://svelte.dev/docs/kit/adapter-node#Environment-variables) for more info.
