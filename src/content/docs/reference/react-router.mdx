...
description: "Reference for the Arcjet React Router SDK: install `@arcjet/react-router`, configure a client, protect a loader or action, and read the decision."
...
Get the Arcjet key for your site from the [Arcjet dashboard](https://console.arcjet.com).
...
For React Router, `requestInput` in the following example is the loader or
action arguments.
...
If one of them is `"DENY"`, then the overall conclusion is `"DENY"`. Otherwise,
if one of them is `"ERROR"`, then `"ERROR"`. Otherwise, it is `"ALLOW"`. The
`reason` and `ttl` fields reflect this conclusion.
...
decisions based on it. For more information, see the <Link.Page
href="/blueprints/ip-geolocation">IP geolocation</Link.Page> and <Link.Page
href="/blueprints/vpn-proxy-detection">IP reputation</Link.Page> blueprints.

## Errors

Arcjet fails open so that a service issue, misconfiguration, or <Link.Page
href="/architecture#timeout">network timeout</Link.Page> does not block
requests. In many cases, log such errors but otherwise treat them as `"ALLOW"`
decisions. The `reason.message` field describes what occurred.
