...
| `inputs`    | `PolicyInputMap \| None`  | Named values built with `server_input.*` or `local_input.*` – see <Link.Page href="/guards/remote-policies">Agent guard remote policies</Link.Page> |
| `metadata`  | `Metadata \| None`        | Nested JSON for correlation and analytics – see [Metadata](#metadata) |
