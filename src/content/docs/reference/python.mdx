...
- `environment` (`str | None`) — Explicit development/production mode
  ...
  `.env` into `os.environ` (e.g. `pydantic-settings`). See <Link.Page href="/reference/python#pydantic-settings-users">Pydantic-settings users</Link.Page>
  below.
...
Python still exports `experimental_ModerateContent`. The name and result shape
may change, and a call may return an error result and fail open. JavaScript
and Go have graduated this rule; see
...
evaluated, so Arcjet fails open), while a **warning** means the signal is intact
and the decision is just reporting a diagnostic. The guard decision exposes:

- `conclusion` — `"ALLOW"` or `"DENY"`. Always check before proceeding.
- `has_failed_open()` — `True` when the conclusion is `"ALLOW"` *only* because a
  rule (or the decision itself) could not be processed — i.e. the security
...
- `warnings` — diagnostics about your request that don't degrade the signal
  (e.g. a stripped invalid metadata key). Informational only; never changes the
  conclusion.
...
See <Link.Page href="/guards/reference#metadata">Guard metadata</Link.Page>
for the full limit table and language-specific notes.
...
Call `await aj.flush()` (async) or `aj.flush()` (sync) at shutdown so the
final batch is sent. See
<Link.Page href="/testing#testing-guard-and-capture-calls">registering a client and the test client</Link.Page>
when `capture()` is too deep to receive a handle.
...
<Link.Page href="/testing#testing-guard-and-capture-calls">Testing Arcjet</Link.Page>.
