...
- **OpenAI Agents Python `FunctionTool`** – `guard_tool`
  ...
  `openai-agents>=0.19.0,<1`). The extra is unpublished on `arcjet`
  0.9.x. Until it is published, pin `arcjet` to
  [`cafe6a66`](https://github.com/arcjet/arcjet-py/commit/cafe6a6671ab7fdde1b78b59971c3fe8ca863a4d).
  The gate is `FunctionTool.tool_input_guardrails` plus
  `reject_content`. The reject payload is JSON of
  `ArcjetDenialResult`. Don't raise from the guardrail.
