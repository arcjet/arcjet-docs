...
- **Claude Managed Agents custom tools** – `guard_custom_tool` plus
  ...
  `arcjet.guard.claude_managed_agents`, `anthropic`). Inbound is
  `user.message` before send. Custom tools are gated on
  `agent.custom_tool_use` before your app executes. Built-in bash
  and files default to `always_allow` with no customer pre-exec.
