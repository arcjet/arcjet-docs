...
which calls `reject_content(...)`, and the Claude Agent SDK Python
helper, which returns JSON in content with `is_error: True`.
...
- **Strands Agents Python `@tool`** – `guard_tool` plus
  ...
  `arcjet.guard.strands_agents`). Authored `@tool` returns JSON of
  `ArcjetDenialResult`. Unwrapped tools are
  `BeforeToolCallEvent.cancel_tool` (`True` or a string). The
  extra pulls the Strands Agents SDK. A peer version floor is not
  published yet. There is no inbound helper.
  `event.interrupt()` is HITL, not policy. This is not
