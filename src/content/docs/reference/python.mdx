...
| `correlation_id`                   | `str`                  | Correlates this decision with a guard call, workflow run, or agent trace. A dedicated, indexable field — not `extra` — and does not affect the decision or its cache key (`arcjet` >= 0.9.0) |
...
is enabled also raises an `ArcjetMisconfiguration`.
