...
`detect_sensitive_info()`, and `filter_request()`. `protect_signup()`
forwards nested `rate_limit`, `bots`, and `email` mappings to those
factories, so each mapping must include `mode` too.
