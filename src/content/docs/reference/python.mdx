...
`detect_prompt_injection()`, `detect_sensitive_info()`, and
`filter_request()`. `protect_signup()` forwards nested `rate_limit`,
`bots`, and `email` mappings to those factories, so each mapping must
include `mode` too.
...
the request claimed to be a well-known crawler (for example, Googlebot) but did
not
