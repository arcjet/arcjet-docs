...
description: "Reference for the Arcjet Python SDK: install `arcjet`, configure a client, protect a FastAPI or Flask route, guard an agent tool call, and read the decision."
prev: false
...
:::caution[Forwarding headers must come from trusted infrastructure]
Arcjet may use `X-Forwarded-For` when it cannot get a public client IP from the
framework request. Clients can spoof this header if they can reach your
application directly or your proxy preserves client-supplied values. Arcjet
continues protecting the request, but produces one warning for the lifetime of
each Arcjet client instance and records
`client_ip_provenance="unverified-header"` at debug level.
...
`proxies`. Client construction rejects invalid proxy entries and a contradictory
combination of `proxies` with `disable_automatic_ip_detection=True`. Protection
rejects invalid manual `ip_src` values;
trust-all ranges (`0.0.0.0/0` and `::/0`) produce a warning. Inspect the result
with `aj.client_ip_details(request)`. If your application selects the address,
set `disable_automatic_ip_detection=True` and pass the same validated `ip_src`
to `client_ip_details()` and every `protect()` call. Never copy an untrusted
forwarding header into `ip_src`.
:::
...
Malformed IP addresses and CIDRs are rejected when the client is created.
Trusting an entire address family with `0.0.0.0/0` or `::/0` is allowed for
compatibility, but emits a warning because every connecting peer is then
treated as trusted.
...
`provenance` is one of `direct`, `platform`, `trusted-proxy`,
`unverified-header`, `manual`, `development`, `request`, or `none`. `verified`
means the SDK tied the source to the request path; it does not certify that
your infrastructure is correctly configured. With debug logging enabled, the
same values are emitted as `client_ip_provenance`, `client_ip_verified`, and
`client_ip_header` facets.
...
HTTP `detect_prompt_injection` accepts only `mode`, which is required.
Omitting `mode` or passing `threshold=` raises `TypeError`. Guard
...
request. A malformed `ip_src` is rejected before a decision is made.

> **Caution:** Syntax validation does not establish provenance. Ensure `ip_src`
> comes from a trusted source. Do not pass a client-controlled header directly;
> doing so could allow clients to choose the IP address used for fingerprinting,
> rate limiting, and other security checks.
...
| --- | --- | --- |
| `key` | `str` | Your site key (required). An empty key raises `ArcjetMisconfiguration`. Unlike the Go SDK, the Python client never reads `ARCJET_KEY` for you |
| `timeout_ms` | `int` | Request timeout. Defaults to 2000 |
| `base_url` | `str \| None` | Override the Arcjet endpoint |
| `logger` | `Logger \| None` | Receives `AJxxxx` diagnostics. Without one, local failures are coalesced |
| `sensitive_info_backend` | `Backend \| None` | On-device backend for sensitive information detection – see [Sensitive information backends](#sensitive-information-backends) |
...
`ArcjetToolDeniedError`, except the OpenAI Agents Python helper,
which calls `reject_content(...)`, the Claude Agent SDK Python
helper, which returns JSON in content with `is_error: True`, and
the Claude Managed Agents helper, which does not send
`user.message` and returns `user.custom_tool_result`, and
the Strands Agents Python helper, which returns a plain
`ArcjetDenialResult` dict.
Unavailability raises `ArcjetUnavailableError`, except on the LangChain
`guard_tool` surface, which raises `ArcjetToolUnavailableError`. That class and
`ArcjetToolDeniedError` subclass LangChain's `ToolException`, so a wrapped
tool's `handle_tool_error` handles them when you configure one.

#### `guard_action()`

`guard_action` wraps any coroutine function, and `guard_action_sync` wraps any
blocking callable. Both take the callable first, then keyword arguments:

```py
from arcjet.guard import guard_action

refund = await guard_action(
    lambda: issue_refund(invoice_id),
...
| Parameter | Type | Description |
| --- | --- | --- |
| `fn` | `Callable` | The action to guard, taking no arguments (required, positional) |
| `action` | `str` | Checkpoint label, which also selects the remote policy (required) |
| `guard` | `ArcjetGuard \| None` | The client. Defaults to the registered one. With nothing registered the call fails open into an error decision, and `on_guard_error` governs from there |
| `actor` | `str \| None` | Who is performing the action |
| `inputs` | `PolicyInputMap \| None` | Values offered for remote policy evaluation |
| `rules` | `Sequence[RuleWithInput]` | Local guard rules. Defaults to none, which still contacts the server |
| `metadata` | `Metadata \| None` | Application context |
| `correlation_id` | `str \| None` | Identifier linking this event to a Sequence |
| `on_guard_error` | `"deny" \| "allow"` | What to do when policy could not be evaluated. Defaults to `"deny"` |

The return value is whatever `fn` returns. A `DENY` raises `ArcjetDeniedError`
and always blocks, whatever `on_guard_error` says, because that option governs
only an unevaluated policy.

To record that an allowed action happened without gating it, use
`capture_action`, which takes `action`, `metadata`, `correlation_id`, and
`decision_id`.

#### Where each helper goes
...
- **Official CrewAI tool calls** – `register_arcjet_hooks`
  ...
  registered.
- **OpenAI Agents Python `FunctionTool`** – `guard_tool`
  (`arcjet[openai-agents]`, `arcjet.guard.openai_agents`,
  `openai-agents>=0.19.0,<1`). The gate is
  `FunctionTool.tool_input_guardrails` plus `reject_content`. The
  reject payload is JSON of `ArcjetDenialResult`. Don't raise from
  the guardrail.
- **Claude Agent SDK Python `@tool`** – `guard_tool` plus
  `guard_hooks` (`arcjet[claude-agent-sdk]`,
  `arcjet.guard.claude_agent_sdk`, `claude-agent-sdk>=0.2.127,<1`).
  Authored `@tool` returns JSON of `ArcjetDenialResult` in content
  with `is_error: True`. Python does not forward
  `structuredContent`. Unwrapped tools are `PreToolUse` deny.
  Inbound is a `UserPromptSubmit` block through `guard_hooks`. There is
  no inbound helper. `can_use_tool` is HITL, not policy.
- **Claude Managed Agents custom tools** – `guard_custom_tool` plus
  `guard_events` (`arcjet[claude-managed-agents]`,
  `arcjet.guard.claude_managed_agents`, `anthropic>=0.92.0,<2`).
  Inbound is `user.message` before send. Custom tools are gated on
  `agent.custom_tool_use` before your app executes. On `DENY`, send
  `user.custom_tool_result` with `is_error`. Built-in bash and
  files default to `always_allow` with no customer pre-exec.
  `always_ask` is HITL, not policy. This is not the Claude Agent
  SDK.
- **Strands Agents Python `@tool`** – `guard_tool` plus
  `guard_hooks` (`arcjet[strands-agents]`,
  `arcjet.guard.strands_agents`, `strands-agents>=1.11.0,<2`).
  Authored `@tool` returns a plain `ArcjetDenialResult` dict.
  Unwrapped tools are `BeforeToolCallEvent.cancel_tool` (`True`
  or a string). Don't set `BeforeToolsEvent.cancel`. There is no
  inbound helper. `event.interrupt()` is HITL, not policy. This
  is not `@arcjet/guard/strands-agents/v1`.
- **Google ADK Python `LlmAgent`** – `guard_tool` plus
  `guard_plugin` (`arcjet[google-adk]`,
  `arcjet.guard.google_adk`, `google-adk>=2.0.0,<3`).
  `LlmAgent.before_tool_callback` and Runner
  `before_tool_callback` return a skip dict with `arcjetDenied`.
  `None` executes. Never return `{}`. There is no inbound
  helper. `require_confirmation` is HITL, not policy. This is
  not `@arcjet/guard/google-adk/v2`.
...
<Link.Page href="/guards/capture#automatic-captures-from-a-framework-wrapper">Automatic captures from a framework wrapper</Link.Page>.
...
hash is sent alongside the local result. The built-in entity types are
...
##### Sensitive information backends

An on-device backend adds entity types that pattern matching alone doesn't
reach, such as `"BANK_ACCOUNT"`, `"ROUTING_NUMBER"`, names, and addresses.
Install the extra, then pass the backend when you create the client:
...
Every `LocalDetectSensitiveInfo` rule on that client uses the backend, and so
does the sensitive information rule in an
<Link.Page href="/guards/remote-policies">agent guard remote policy</Link.Page>.
The first guarded call loads the model, so it takes noticeably longer than the
ones after it.
...
| Parameter   | Type                      | Description |
| ----------- | ------------------------- | ----------- |
| `label`     | `str`                     | Label identifying this guard call (required). A slug: lowercase letters, digits, dash (`-`), dot (`.`), and underscore (`_`); must start and end with a lowercase letter or digit |
| `rules`     | `Sequence[RuleWithInput]` | Bound rule inputs. Defaults to empty, which still contacts Guard so a remote policy selected by `label` can apply |
| `actor`     | `str \| None`             | Trusted, application-asserted identity. The SDK never derives it from a request or from tool arguments |
| `inputs`    | `PolicyInputMap \| None`  | Named values built with `server_input.*` or `local_input.*` – see <Link.Page href="/guards/remote-policies">Policy contract</Link.Page> |
| `metadata`  | `Metadata \| None`        | Nested JSON for correlation and analytics – see [Metadata](#metadata) |
...
`"tools.get-weather"`, not `f"tools.{name}"`). Labels are slugs – lowercase
letters, digits, dash (`-`), dot (`.`), and underscore (`_`) – and must start
and end with a lowercase letter or digit. Uppercase is rejected, so use
`tools.get-weather` or `tools.get_weather`, not `tools.getWeather`. Hardcoded
labels stay greppable and the dashboard groups by them. Pass `metadata`
whenever you have useful auditing context – nested objects and arrays are
accepted, and it shows up in the dashboard.

A label the service will not match reads as `ALLOW` with `has_failed_open()`
false, so the guard does not run. Adapter factories raise
`ArcjetInvalidLabelError` (`arcjet` >= 1.2.0). `capture()` warns `AJ1023` and
still sends. Check a label you build yourself with `validate_guard_label`.
