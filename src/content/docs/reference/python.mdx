...
- `has_failed_open()` — `True` when the conclusion is `"ALLOW"` *only* because a
  rule (or the decision itself) could not be processed. This is the fail-closed
  gate: deny on it for sensitive operations. Errors fail open — the SDK never
  blocks on them by itself (`arcjet` >= 0.9.0).
- `error_results()` — the errored rule results (each with a `code` / `message`)
  for logging (`arcjet` >= 0.9.0).
- `warnings` — request-validation diagnostics (e.g. a stripped metadata key).
  Informational only; never changes the conclusion.
