...
import SDKVersionRemix from "/src/components/SDKVersionRemix.astro";
...
The Arcjet Remix SDK works with Remix environment variable support.
Remix loads `.env` automatically in development.
See [“Environment variables” in the Remix
docs](https://v2.remix.run/docs/guides/envvars) for more info.

The Arcjet Remix SDK uses several environment variables to configure its behavior.
...
See the [rate limiting documentation](/rate-limiting/reference?f=remix) for more
