...
You can configure Arcjet to trust the `X-Forwarded-For` header by setting the
`proxies` field in the configuration. This should be a list of the IP addresses
of your load balancers to be removed, so that the last IP address in the list is
