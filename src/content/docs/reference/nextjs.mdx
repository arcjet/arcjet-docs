...
The Arcjet Next.js SDK works with Node.js environment variable support
(`process.env`).
Next.js can load `.env` automatically when using `loadEnvConfig` from
`@next/env`.
See [“How to use environment variables in Next.js” on
`nextjs.org`](https://nextjs.org/docs/pages/guides/environment-variables) for more info.

The Arcjet Next.js SDK uses several environment variables to configure its behavior.
