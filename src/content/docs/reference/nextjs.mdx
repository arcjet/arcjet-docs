...
rules. The order of priority is allow, then error, then deny.

For example, if the bot protection rule returns an error but the validate email
rule returns a deny then the overall conclusion would be deny. To then access the error
...
to see other rules consider iterating over the `results` property on the decision.
