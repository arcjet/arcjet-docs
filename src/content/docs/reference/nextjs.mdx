...
description: "Reference for the Arcjet Next.js SDK: install `@arcjet/next`, configure a client, protect a route handler, page, server action, or middleware, and read the decision."
prev: false
...
import CloudflareTS from "/src/snippets/reference/nextjs/Cloudflare.ts?raw";
...
[<NpmSdkVersion packageName="@arcjet/next" />](https://www.npmjs.com/package/@arcjet/next)
...
:::note
If you use Bun to run your Next.js app,
...
This happens because Next.js bundles for Node.js but Bun does not support all
Node.js APIs.
To solve this configure Webpack in Next.js to bundle for Bun by adding
the following to your `next.config.js`:
...
This may require running Next.js with the `--webpack` flag.
Turbopack does not support conditions.
:::
...
See the <Link.ToSdk href="/get-started" sdk="next">quick start guide</Link.ToSdk>.

## Configuration

Create a new `Arcjet` object with your API key and rules. Create it outside of
the request handler.
...
- `rules` - The rules to apply to the request. See the various sections of the
  ...
  <Link.ToSdk href="/shield/reference" sdk="next">shield</Link.ToSdk>,
  <Link.ToSdk href="/rate-limiting/reference" sdk="next">rate limiting</Link.ToSdk>,
  <Link.ToSdk href="/bot-protection/reference" sdk="next">bot protection</Link.ToSdk>,
  <Link.ToSdk href="/email-validation/reference" sdk="next">email validation</Link.ToSdk>.
...
The Arcjet Next.js SDK uses several environment variables to configure its
...
Arcjet supports server actions in Next.js for all features except sensitive data
detection. You need to call a utility function `request()` that accesses the
headers we need to analyze the request.
...
These examples `throw` when there is an error, as described in the
[Next.js error handling
documentation](https://nextjs.org/docs/app/building-your-application/data-fetching/server-actions-and-mutations#error-handling),
but you can use
...
href="/shield/reference" sdk="next">shield documentation</Link.ToSdk>.
...
For more information about these properties, see the <Link.ToSdk href="/rate-limiting/reference" sdk="next">rate limiting documentation</Link.ToSdk>.
...
For more information about these properties, see the <Link.ToSdk href="/email-validation/reference" sdk="next">email validation documentation</Link.ToSdk>.
...
a default one. You don't usually need to provide a client – the Arcjet Next.js
SDK handles this for you, including when you use the
...
router. When a new major version of Next.js is released, we bump the major
version of the Arcjet SDK.

Maintenance LTS: Next.js 15.x
Active LTS: Next.js 16.x
