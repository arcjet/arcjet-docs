...
- `rules` - The rules to apply to the request. {/* See the various sections of the */}
  {/* docs for how to configure these e.g. <Link.ToSdk href="/shield/reference" sdk="next">shield</Link.ToSdk>, [rate */}
...
for more info.
The `ARCJET_KEY` environment variable is read automatically.
The value at `import.meta.env.MODE` is also read to determine whether the app is
in development mode.
The `NODE_ENV` environment variable is not used.
