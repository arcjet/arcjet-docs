...
dynamic pages, see [Static routes](#static-routes) below for more information.
...
Arcjet can protect Astro server endpoints (API routes). Keep in mind that Arcjet
is designed to protect dynamic endpoints, see [Static routes](#static-routes)
...
:::note
Depending on your on-demand rendering server adapter, Astro's middleware may
only run at build time for static routes. It is important to **only call
`protect` when `context.isPrerendered === false`**. See [Static routes](#static-routes) below and server adapters in the [Astro docs](https://docs.astro.build/en/guides/on-demand-rendering/#server-adapters)
