...
description: "Reference for the Arcjet Astro SDK: install `@arcjet/astro`, configure a client, protect a route or middleware, and read the decision."
prev: false
...
import CloudflareConfigJS from "/src/snippets/reference/astro/Cloudflare.mjs?raw";
...
[<NpmSdkVersion packageName="@arcjet/astro" />](https://www.npmjs.com/package/@arcjet/astro)
...
:::note
If you use Bun to run your Astro app,
...
This happens because Astro bundles for Node.js but Bun does not support all
Node.js APIs.
To solve this configure Vite in Astro to bundle for Bun by adding the following
...
See the
<Link.ToSdk href="/get-started" sdk="astro">quick start guide</Link.ToSdk>.

## Configuration

Arcjet is configured as an integration in your `astro.config.mjs` file. Add your
...
- `rules` - The rules to apply to the request. {/* See the various sections of the */}
  {/* docs for how to configure these, such as <Link.ToSdk href="/shield/reference" sdk="next">shield</Link.ToSdk>, [rate */}
...
The integration validates each rule with Zod `.strict()`. Unknown fields throw
at startup. `detectPromptInjection` accepts only `mode`. Leftover `threshold`
in this config throws; the core JS SDK ignores it instead.
...
The Arcjet Astro SDK uses several environment variables to configure its
...
is read automatically. The value at `import.meta.env.MODE` is also read to
determine whether the app is in development mode. The `NODE_ENV` environment
...
{/* See the <Link.ToSdk href="/shield/reference" sdk="astro">shield documentation</Link.ToSdk> for more information */}
...
{/* See the <Link.ToSdk href="/rate-limiting/reference" sdk="astro">rate limiting documentation</Link.ToSdk> for more */}
...
{/* See the <Link.ToSdk href="/email-validation/reference" sdk="astro">email validation documentation</Link.ToSdk> for */}
...
Arcjet supports Astro v5.9.3 and later. We follow [Astro's support
