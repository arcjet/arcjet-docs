...
Plain IP or CIDR entries in `proxies` tell Arcjet which hops to skip; they do not
verify that the connection came through those proxies.
...
`proxies`. Use a proxy-service helper such as `cloudflare()` where available. If
your application determines the client IP itself, pass a validated `ipSrc` to
`protect()`.
:::
