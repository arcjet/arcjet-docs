...
The Arcjet Node.js SDK works with Node.js environment variable support
(`process.env`).
Node.js can load `.env` automatically when running with the `--env-file=file`
flag.
See [“Environment variables” in the Node.js
docs](https://nodejs.org/docs/latest/api/environment_variables.html) for more info.

The Arcjet Node.js SDK uses several environment variables to configure its behavior.
