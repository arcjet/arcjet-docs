...
description: "Reference for the Arcjet Node.js SDK: install `@arcjet/node`, configure a client, protect an HTTP, Express, or Hono handler, and read the decision."
next: false
...
import CloudflareTS from "/src/snippets/reference/nodejs/Cloudflare.ts?raw";
...
[<NpmSdkVersion packageName="@arcjet/node" />](https://www.npmjs.com/package/@arcjet/node)
...
See the <Link.ToSdk href="/get-started" sdk="node">quick start guide</Link.ToSdk>.
...
reach your application directly or your proxy preserves client-supplied values.
Arcjet continues protecting the request, but produces one warning for the
lifetime of each Arcjet client instance and records
...
overwrites or safely appends forwarding headers, and list every trusted hop in
`proxies`. Use a proxy-service helper such as `cloudflare()` where available.
Invalid proxy entries and non-empty malformed manual `ipSrc` values are
rejected; an empty `ipSrc` is treated as omitted. Trust-all ranges produce a
warning. Inspect the selection with `client.clientIpDetails(request)`
in `@arcjet/node`, or `findIpDetails()` / `resolveClientIp()` from `@arcjet/ip`.
Never copy an untrusted forwarding header into `ipSrc`.
:::
...
- `rules` - The rules to apply to the request. See the various sections of the
  ...
  <Link.ToSdk href="/shield/reference" sdk="node">shield</Link.ToSdk>,
  <Link.ToSdk href="/rate-limiting/reference" sdk="node">rate limiting</Link.ToSdk>,
  <Link.ToSdk href="/bot-protection/reference" sdk="node">bot protection</Link.ToSdk>,
  <Link.ToSdk href="/email-validation/reference" sdk="node">email validation</Link.ToSdk>.
...
The Arcjet Node.js SDK uses several environment variables to configure its
...
Trusting `0.0.0.0/0` or `::/0` is allowed for compatibility, but emits a warning
because every connecting peer is then treated as trusted.
...
protection rules. This requires a `request` object, which you must construct
with your incoming request. Rules you add to the SDK may require additional
details, such as the `validateEmail` rule requiring an additional `email` prop.
...
href="/shield/reference" sdk="node">shield documentation</Link.ToSdk>.
...
Every resolution has an `ip`, `provenance`, `verified`, and optional `header`.
The provenance is `direct`, `platform`, `trusted-proxy`, `unverified-header`,
`manual`, `development`, `request`, or `none`. Enable debug logging to receive
the same information in `client_ip_provenance`, `client_ip_verified`, and
...
Other framework adapters can inspect their request-like object with
`findIpDetails(request, options)` or reproduce manual/development resolution
with `resolveClientIp(request, options)` from `@arcjet/ip`. `verified` means the
SDK tied the source to the request path; it does not certify your deployment.
...
a default one. You don\'t usually need to provide a client – the Arcjet SDK
