...
- **`list-requests`** – Lists recent requests for a site within the plan retention window. Supports filtering by conclusion (`ALLOW`, `DENY`, `ERROR`) and pagination.
- **`get-request-details`** – Returns full details for a specific request inside the plan retention window, including headers, rules executed, and decision information.
