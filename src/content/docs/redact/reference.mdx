...
The Arcjet Redaction library can be used to both redact and unredact text. First you
redact using the `redact()` function which returns both the redacted text and an `unredact()`
function as a tuple. You can use the `unredact()` function later on to replace redacted entities
...
You can provide a function to perform custom entity redaction if have different requirements
for the redacted entities. A common example of this is to use a library such as faker.js to
generate redacted entities that look exactly like the real ones.
