...
handlers. Do not use it to wrap Mastra `createTool`, Eve `defineTool`, LangGraph
...
- `guardTool()` wraps an AI SDK `tool({ execute })`. On `DENY` the tool never
  ...
  (`{ arcjetDenied, reason, message, retryable, retryAfterSeconds? }`). Do not
  throw.
...
- `createAgentContext()` builds the context you pass to `aiToolsContext`. Pass a
  stable `correlationId` when the app already has one. Omit it and a ULID is
  generated.
...
it by hand into `generateText` and any `guardAction` call. Do not store it in
...
`guardTool` already defaults to fail closed. You do not need this extra check
on wrapped tools.
...
try {
  await guardAction(
    ...
    context,
    {
      ...
      rules: [submitLimit({ key: repoId, requested: 1 })],
      metadata: securityMetadata({
        destination: "github",
...
- Do not pass `label` to `guardTool`. The field is `action`.
- Do not declare a `contextSchema` on a tool you wrap. Arcjet uses that slot.
- Do not wrap the same tool twice. `guardTool` throws if the tool already
  carries the Arcjet protection brand.
- Do not also wrap these tools with another framework adapter.
- Do not import `@arcjet/guard/vercel-ai`. The path is
  `@arcjet/guard/vercel-ai/v7`.
- Do not use this adapter with Mastra, Eve, LangGraph, OpenAI Agents, or Claude
  Agent SDK tools.
