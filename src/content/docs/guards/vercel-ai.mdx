...
`tool()`, OpenAI Agents `tool()`, Claude Agent SDK `tool()`, or official CrewAI
`@tool` – each of those has its own adapter.
...
peers. The integration requires Node.js `>=22.21.0 <23 || >=24.5.0`.
...
| --- | --- | --- |
| `action` | Yes | Guard label and capture name. Use `resource.verb` in the past tense, such as `email.sent`. Slug: lowercase letters, digits, dash, underscore, and dot. Must start and end with a lowercase letter or digit. |
...
## The policy that reads your inputs

The names in `inputs` are the contract. A policy reads them under
`input.values`, so a policy that declares `recipient` as a `SERVER` string and
`allowed_recipients` as a `SERVER` string list denies a recipient that isn't on
the list like this:
...
A mistyped name produces a policy that silently never fires, so generate the
guard call from the policy rather than transcribing it. Every policy page in
the Console generates the call that sends its inputs.
...
- Don't use this adapter with Mastra, Eve, LangGraph, OpenAI Agents, Claude
  Agent SDK, or official CrewAI tools.
