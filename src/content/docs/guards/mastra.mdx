...
Vercel AI SDK, LangChain, CrewAI, Eve, and LangGraph wrappers are on
...
integration requires `@mastra/core` 1.x and Node.js `>=22.21.0 <23 || >=24.5.0`.
...
Mastra `requireApproval` pauses for a human. It asks a person rather than
evaluating a policy, so it is not a Guard enforcement point. For more
information, see
...
- There is no `guardApproval`. `requireApproval` is human HITL confirmation,
  not policy.
...
`guardHooks` resolvers receive `{ toolName, input }`, and `guardProcessor`
resolvers receive `{ text, messages }`, each followed by the Mastra request
context.
