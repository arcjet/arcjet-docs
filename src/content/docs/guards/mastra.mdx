...
processors, and hooks. Vercel AI SDK, LangChain, Eve, and LangGraph wrappers are
on
...
On `DENY` the tool's `execute` never runs. The model receives `{ arcjetDenied:
true, reason, message, retryable }`. Scan free-text args (a note, reason, or
body). An opaque `orderId` doesn't trip email, phone, card, or IP detection, so do not
