...
Use <Link.Page href="/get-started">`protect()`</Link.Page> for HTTP routes. Use the Mastra helpers on this page for tools,
processors, and hooks. Vercel AI SDK, LangChain, and Eve wrappers are on
...
integration requires `@mastra/core` >=1 <2 and Node.js 22 or later.
...
- `guardHooks()` provides `beforeToolCall` / `afterToolCall` for MCP /
  workspace / toolset tools that were not authored through `guardTool`.
...
Prompt injection belongs on `guardProcessor` in `inputProcessors`.
...
note, reason, or body). An opaque `orderId` will not trip email / phone / card
/ IP, so do not pass it to `localDetectSensitiveInfo`.
...
`guardHooks` for MCP / workspace / toolset tools you did not pass through
