...
| `correlation_id` | No | Caller-owned UUID. Falls back to the enclosing `arcjet_sequence`. |
...
| `actor` | No | Who is acting, or a function of the hook input. Take it from authenticated application state, never from a model-produced argument. |
| `inputs` | No | Typed remote-policy inputs, or a function of the hook input. |
| `rules` | No | Bound SDK rule inputs, or a function of the tool arguments. Omit to submit none. |
| `metadata` | No | Capture metadata, or a function of the hook input. |
| `inbound` | No | Policy for `UserPromptSubmit`. Requires `action`. `rules` receives `{"prompt": ...}`. |
| `tools` | No | Pass `True` to register the tool hooks without giving them a policy of their own. |
...
`guard_hooks` needs a tool policy (`action`, `rules`, `exclude`, or
`tools=True`), an `inbound` policy, or both. There is no `guard_inbound`.

The Python helper registers only what you ask for. Giving it `inbound` alone
registers `UserPromptSubmit` and leaves tool calls ungated. The JavaScript
`guardHooks` always registers `PreToolUse`, `UserPromptSubmit`, and
`PostToolUse`, so the same-looking call gates every tool as well.
...
Claude permission callback, so the callback looks like a gate that the runtime
can step around. It also asks a person rather than evaluating a policy. For
...
`guardHooks` takes the same two options. Its tool resolvers receive
`{ toolName, input }` and its inbound resolvers receive `{ prompt }`, each
followed by the hook input.
...
- JavaScript example:
  [examples/claude-agent](https://github.com/arcjet/examples/tree/main/examples/claude-agent)
- Python example:
...
- Python adapter:
  [`0fa5f546`](https://github.com/arcjet/arcjet-py/commit/0fa5f54670a74cc75800225cc316d3e0c370355e)
