...
Eve, Mastra, and LangGraph wrappers are on
...
// Key the budget on the authenticated caller, not on a model-supplied
// object id. An object-keyed budget resets when the agent varies the id,
// and unrelated callers share one bucket.
...
The Claude Agent SDK CLI requires `options.sessionId` to be a UUID, and you
can create that id only once. A non-UUID value exits the CLI with
`Invalid session ID. Must be a valid UUID.` Passing the same id to a second
`query()` call exits with `Session ID … is already in use.`

Mint one UUID per conversation. On later turns, pass `resume: sessionId`
instead of `sessionId`. `claudeAgentContext` then reads the hook's
`session_id` and joins those turns to the same conversation.

`claudeAgentContext` never calls `createAgentContext`. If neither hook
`session_id` nor `options.sessionId` is a valid 1-256 printable-ASCII
...
- Do not mint a new `sessionId` on every turn. Continue with `options.resume`.
