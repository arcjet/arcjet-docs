...
| `AJP1009` | An Arcjet threat intel lookup couldn't be completed          | Transient or a missing IP database binding. Every rule that reads the detector reports it. A host already scored high or critical still returns that verdict when another lookup fails. |
...
model-backed detector, `AJP1009` for an Arcjet threat intel lookup – not only
the rules that name it. An expression reading
