...
Agent guard remote policies currently support:
...
remote rate limits are not currently supported. SDK rules configured in code
...
One `email.sent` policy can combine all three rules above. The integration maps
...
See <Link.Page href="/guards/reference#how-arcjet-fails-when-a-security-check-cannot-finish">How Arcjet fails when a security check cannot finish</Link.Page>.
