...
`has_failed_open()` false, so the guard does not run. Adapter factories throw
or raise `ArcjetInvalidLabelError` (`@arcjet/guard` >= 1.13.0, `arcjet` >=
1.2.0). `capture()` warns `AJ1023` and still sends. Check a label you build
yourself with `validateGuardLabel` / `validate_guard_label`.
...
The server-side detector screens values up to 2&nbsp;KB. A larger value is
...
Both kinds write their result to `input.signals.sensitive_info`, so moving a
