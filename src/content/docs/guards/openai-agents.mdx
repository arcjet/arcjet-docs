...
Import helpers from the versioned path `@arcjet/guard/openai-agents/v0`. There
is no unversioned alias. `@arcjet/guard/openai-agents` does not resolve. The
SDK is pre-1.0, so the segment is `v0`. `@openai/agents` (`>=0.17.0 <1`) is an
optional peer. The integration requires Node.js `>=22.21.0 <23 || >=24.5.0`.
...
| `actor` | `guardTool` | Trusted identity, or a function resolved per call. Take it from authenticated application state, never from a model-produced argument. |
| `inputs` | `guardTool` | Named values built with `policyInput.server.*` or `policyInput.local.*`, or a function resolved per call. |
...
| `tool` | Yes | The `FunctionTool` to gate. |
...
| `correlation_id` | No | Caller-owned correlation ID. Falls back to the enclosing `arcjet_sequence`. |
...
for human-in-the-loop (HITL) confirmation, and hosted MCP approval is the same
class of control. Neither evaluates a policy, so neither is a Guard
