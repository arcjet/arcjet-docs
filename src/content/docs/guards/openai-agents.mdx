...
description: "Screen user text before the agent run, wrap authored function-tool invoke, and treat approval pauses as human-in-the-loop confirmation, not policy."
---
...
Agents Python, and Claude wrappers are on
...
For more information about the Python `openai-agents` adapter, see
...
human-in-the-loop (HITL) confirmation (`state.approve` / `state.reject`).
Hosted MCP `requireApproval` is the same class of control. Neither is a remote
policy. Same trap as Mastra `requireApproval`, Claude `canUseTool`, and
LangGraph `interrupt()`. There is no `guardApproval`. Don't wrap them as Guard.
...
It does not throw. A throw hits `errorFunction` or `ToolCallError` and
drops the fields. The runner stringifies that object onto a
`function_call_result` with `status: "completed"`. Don't throw from
`execute` to signal a denial. `timeoutMs` races the guard round trip, and
...
## Deny authored `tool({ execute })` handlers
...
- There is no `guardApproval`. `needsApproval` and hosted `requireApproval`
  are human HITL confirmation, not policy.
...
- **Prompt injection before the run:** Direct `guard()` with
...
- **Correlate the run:** Put `{ sessionId: conversationId }` on
  `run(..., { context })`. Don't read `traceId` or call `getSessionId()`.
