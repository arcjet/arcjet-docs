...
| `actor` | `guardTool`, `guardHooks` | Trusted identity, or a function resolved per call. Take it from authenticated application state, never from a model-produced argument. |
| `inputs` | `guardTool`, `guardHooks` | Named values built with `policyInput.server.*` or `policyInput.local.*`, or a function resolved per call. |
...
| `actor` | No | Trusted identity, or a callable of the parsed tool arguments. |
| `inputs` | No | Named values built with `server_input.*` or `local_input.*`, or a callable of the parsed tool arguments. |
...
| `actor` | No | Trusted identity, or a callable of the tool-call envelope. |
| `inputs` | No | Named values built with `server_input.*` or `local_input.*`, or a callable of the tool-call envelope. |
...
`guardHooks` takes the same two options. Its resolvers receive
`{ toolName, input }`, then the Strands context.
...
`register_arcjet_hooks` takes them too, with callables of the tool-call
envelope.
