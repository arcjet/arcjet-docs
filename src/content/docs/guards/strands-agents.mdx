...
is not in the published `@arcjet/guard` package on npm. The module
is unpublished and will be installed from git once an adapter SHA
is provided. There is no adapter SHA yet. Replace `ADAPTER_SHA` in
the command that follows when one is provided. Do not invent a
commit hash. The published package does not export
`./strands-agents/v1`.
...
- `guardTool()` wraps a Strands `tool()` you pass to
  `new Agent({ tools })`. On `DENY` the original `callback` never
  runs. The helper returns a plain `ArcjetDenialResult`. It does
  not throw. It does not call `event.interrupt()`.
- `guardHooks()` is the invoke-wide `BeforeToolCallEvent` gate.
  Register the return value with
  `agent.addHook(BeforeToolCallEvent, hooks)`. On `DENY` it sets
  `event.cancel` to a JSON string of `ArcjetDenialResult`. That
  string is the tool result error message. It does not call
  `event.interrupt()`. It skips branded (`guardTool`) tools when
  the tool can be looked up. Tools that cannot be looked up are
  still gated.
- `strandsAgentContext()` reads caller-owned
  `invocationState.sessionId`, then `correlationId`, then
...
| `sessionId` | `guardTool`, `guardHooks` | Caller-owned fallback when `invocationState` has no `sessionId`, `correlationId`, or `requestId`. A string, or a function of the same input as `rules`. Prefer putting the ID on `agent.invoke(prompt, { invocationState })`. |
...
Screen inbound text, wrap `lookup_order`, and register
...
`guardHooks` is the invoke-wide `BeforeToolCallEvent` gate.
Register it with `agent.addHook(BeforeToolCallEvent, hooks)`. The
callback denies by setting `event.cancel` to a JSON string of
`ArcjetDenialResult` without calling the tool. Policy sits on
`cancel` only. It skips branded (`guardTool`) tools when the tool
can be looked up. Tools that cannot be looked up are still gated.

MCP and injected tools skip an unwrapped handler. Hosted or
injected tools are not a `tool()` deny. `guardHooks` still gates a
tool call that Strands executes through `BeforeToolCallEvent`.
...
`strandsAgentContext` reads caller-owned
`invocationState.sessionId` first, then `correlationId`, then
`requestId`. It never mints an ID. It never reads `traceId`. It
never calls `createAgentContext`. If the caller did not put one of
those keys on `invocationState`, the call is uncorrelated rather
...
- Don't turn a deny into `event.interrupt()` or a human approval
  pause.
- Don't mint a correlation ID. Don't read `traceId`. If
  `invocationState` has no `sessionId`, `correlationId`, or
  `requestId`, leave the call uncorrelated.
