...
- **Custom tools on `agent.custom_tool_use`:** Wrap the handler your
  ...
  `user.custom_tool_result`. Don't throw.
...
The peer packages are `@anthropic-ai/sdk` (JavaScript) and `anthropic`
(Python). Don't install `claude-agent-sdk` or
...
```bash
npm install @arcjet/guard@git+https://github.com/arcjet/arcjet-js.git#ADAPTER_SHA @anthropic-ai/sdk
```

`@anthropic-ai/sdk` is an optional peer, not a dependency of
`@arcjet/guard`. If your project already has it, install
`@arcjet/guard` on its own so your pins don't move.
...
Until `arcjet.guard.claude_managed_agents` is published, pin `arcjet`
to `ADAPTER_SHA`. Published `arcjet` 1.0.0 on PyPI does not include
this extra.

```bash
pip install "arcjet[claude-managed-agents] @ git+https://github.com/arcjet/arcjet-py@ADAPTER_SHA"
```

`arcjet[claude-managed-agents]` depends on `anthropic`. CPython 3.10
or later.
...
`user.custom_tool_result`. Don't throw. A throw is a raw exception.
...
with the denial payload so the model can inspect it. Don't throw. A
...
- Don't throw from `guardCustomTool` / `guard_custom_tool` to
  signal a denial. Return `user.custom_tool_result`.
...
- Adapter JS: `ADAPTER_SHA`
- Adapter Python: `ADAPTER_SHA`
