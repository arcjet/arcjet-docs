...
Genkit, Eve, Mastra, OpenAI Agents JS, Strands Agents, and Claude
wrappers are on
<Link.Page href="/guards/framework-integrations">Framework integrations</Link.Page>.

This adapter is the Python `openai-agents` text `Agent` plus
`Runner.run` plus an authored `FunctionTool`. Don't wrap these tools
with `@arcjet/guard/openai-agents/v0` or
