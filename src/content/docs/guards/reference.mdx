...
A rule carries its own metadata, and the metadata you configure on the rule
merges shallowly with the metadata you bind at call time: a duplicate key's
whole value is replaced, never deep-merged. Metadata you pass to `guard()`
travels on the request instead, and is never merged into a rule's.
...
`hasFailedOpen()`. Framework wrappers also write `metadata.outcome` on the
event they emit. For options, batching, `flush()`, serverless `waitUntil`, and
the outcome values, see <Link.Page href="/guards/capture">Capture events</Link.Page>.
...
### Policy rule result types

The type of a policy rule result follows the kind of the rule that produced it:

| Result type         | Produced by                                                            |
| ------------------- | ------------------------------------------------------------------------ |
| `POLICY_EXPRESSION` | A rule decided by the policy's Rego. Carries the rule's conclusion.    |
| `PROMPT_INJECTION`  | A rule decided directly by a prompt injection detector.                |
| `SENSITIVE_INFO`    | A rule decided directly by a sensitive information detector.           |
| `ERROR`             | A rule that couldn't be evaluated. Carries a stable code and message.  |

`POLICY_EXPRESSION` is the type most rules produce, and it carries the
conclusion and nothing more. Which conditions fired is a property of the
compiled program named by the policy revision and artifact digest, not
something the result restates. Record the artifact digest alongside the
decision ID when you need to reconstruct an answer later.

An SDK released before this result type reads it as an unset variant. It
enforces the decision correctly, because enforcement follows the decision's
conclusion rather than per-rule detail, but it reports no detail for that rule
until you upgrade.

An `ERROR` result carries an `AJP` code naming why the rule couldn't be
evaluated. For the codes and how to handle each one, see
...
`policyEvaluation` / `policy_evaluation` also carries `artifactSha256` /
`artifact_sha256`, a digest of the compiled program that evaluated. It's empty
when no expression rule ran, which is the case for a policy decided entirely by
detectors. Record it alongside the decision ID when you need to prove later
which program produced an answer.
...
The `_sync` suffix belongs to the free functions, not to the client. A client
you hold has the same method names in both flavors, so a blocking client
evaluates a policy with `arcjet.guard(...)` and flushes with
`arcjet.flush()`. There is no `arcjet.guard_sync(...)` method.
