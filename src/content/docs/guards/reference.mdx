...
Rules in one `guard()` call are all evaluated. A denial from one rule still
spends a rate limit in the same `rules` array. A request that trips
`localDetectSensitiveInfo` also consumes a token from a `tokenBucket` in
that array. The rate-limit result is a `TOKEN_BUCKET` `ALLOW` with a
decremented `remaining` on a decision whose conclusion is `DENY`. Split the
rules across two `guard()` calls if a false positive must not drain that
budget.
...
accessors. In JavaScript, `reason` is typed `Reason | undefined`: a broad
category such as `RATE_LIMIT` on `DENY`, and `undefined` on `ALLOW`. Check
`conclusion` before you treat `reason` as a string. In a `strict` TypeScript
build, assigning `decision.reason` to a `string` fails because the value is
`undefined` on `ALLOW`. Remote policy status and results are additive and
may be absent when the server does not provide policy data.
