...
| `actor` | `guardTool`, `guardMiddleware` | Trusted identity, or a function resolved per call. Take it from authenticated application state, never from a model-produced argument. |
| `inputs` | `guardTool`, `guardMiddleware` | Named values built with `policyInput.server.*` or `policyInput.local.*`, or a function resolved per call. |
...
`guard_tool`, `guard_action`, and `ToolPolicy` share most of one contract:
...
| `metadata` | No | Capture metadata. |

Where they differ:

- `guard_tool` also requires `guard` and `tool`, and accepts `on_guard_error`.
- `guard_action` also accepts `guard`, `correlation_id`, and `on_guard_error`.
- `ToolPolicy` carries policy only. Its fail-closed behavior comes from the
  `on_guard_error` you pass to `ArcjetMiddleware`.
...
It asks a person rather than evaluating a policy, so it is not a Guard
enforcement point. For more information, see
...
Wrappers that sit around an effect record `metadata.outcome` on the
capture event. If the action ran only because `onGuardError` /
`on_guard_error` is `"allow"` and policy did not judge it fully, the
event records `degraded`. The default records `unavailable` and
blocks. For more information about the five values and how a decision
ID distinguishes a partial judgment, see
...
`guardTool` takes `actor` and `inputs`, each as a value or a function
resolved per call. A resolver receives the parsed tool input, then the agent
context.
...
`guardMiddleware` takes the same two options. Its resolvers receive
`{ toolName, input }`, then the LangChain context.

</TabItem>
<TabItem label="Python">

`guard_tool` takes `actor` and `inputs`. `inputs` is a value or a callable of
`(arguments, config)`; `actor` is a value or a callable of the `RunnableConfig`.
