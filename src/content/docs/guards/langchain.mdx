...
Vercel AI SDK, LangChain JS, CrewAI, LangGraph JS, Eve, Mastra, and Claude
Agent SDK wrappers are on
<Link.Page href="/guards/framework-integrations">Framework integrations</Link.Page>.

This is the Python SDK. For more information about the JavaScript
`createAgent` adapter, see
<Link.Page href="/guards/langchain-js">LangChain JS agent guard</Link.Page>.
It is not the JavaScript LangGraph Graph API adapter
(`StateGraph` + `ToolNode`).
