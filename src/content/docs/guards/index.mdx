...
# has_error() means a rule errored or the server reported diagnostics.
# The SDK failed open — log it but don't block the caller.
if decision.has_error():
    logger.warning("Arcjet rule error")
```
...
- **Hardcode the `label` string.** Use `"tools.get_weather"`, not
...
- **Branch on the denied result, not just `DENY`.** Specific error messages
  ...
  generic "blocked").
- **Fail open on errors.** `decision.hasError()` (JS) / `decision.has_error()`
  (Python) means a rule errored but the SDK didn't block. Log it; don't deny.
