...
## Deny a high-risk destination

| Input           | Kind        | Exposure |
| --------------- | ----------- | -------- |
| `destinations`  | String list | `SERVER` |

**Detector:** `dest`, Arcjet threat intel over `destinations`
...
Map the hosts the action is about to contact into `destinations`. An empty list
is risk `none`. Name the set of levels you deny – `>= "high"` is wrong because
`critical` sorts before `high`.

On a coding agent hook, Arcjet fills `destinations` from URL arguments and from
absolute URLs inside a shell command.

### Allowlist that bypasses destination threat

When a reviewed host must pass even if threat intel scores it high or
critical, put the allowlist and the threat rule in **one** policy. Do not
publish `coding-agent.egress-allowlist` and `coding-agent.destination-threat`
as two LIVE policies for that goal: most restrictive wins, so threat DENY still
blocks an allowlisted host. The starters cannot express the bypass alone.

| Input           | Kind        | Exposure |
| --------------- | ----------- | -------- |
| `tool_kind`     | String      | `SERVER` |
| `domains`       | String list | `SERVER` |
| `destinations`  | String list | `SERVER` |
...
For signal fields and fail-closed behavior, see
...
- <Link.Page href="/coding-agents/policies">Coding agent policies</Link.Page> – the fixed contract and starter policies
