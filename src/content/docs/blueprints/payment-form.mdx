...
Credit card processors can use other signals to detect fraud once they have the
full transaction details. We recommend using Arcjet as a first defense to block
the most obvious fraud attempts.

## Pre-checkout protection

A common approach for handling payments is to generate a unique payment or
checkout link, such as [Stripe
...
In this example, Arcjet analyzes the user request before checkout. If the
request is high risk, Arcjet blocks the user. If the request is considered safe,
you can then proceed to generate the payment link and redirect the user to the
payment processor.
...
<Link.Page href="/blueprints/vpn-proxy-detection">VPN and proxy detection</Link.Page> checks to further validate the
transaction.
...
This example shows a form component which implements the preceding rules. It
uses the Next.js app router and [shadcn/ui](https://ui.shadcn.com/).
...
The following example is the payment form component. If the form passes the
Arcjet checks, it redirects the user to the link for the payment processor.
