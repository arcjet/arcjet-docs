...
import {detectBot, slidingWindow,validateEmail} from "@arcjet/next";
...
Additionally, you can use the Redact library to ensure no sensitive information is input by the user:

```ts
const [redacted] = await redact(feedback, {
