...
estimate of token consumption. This lets you enforce user allowances and keep
...
We recommend using a Token bucket rate limit. Configure this to match your
category quota, such as tokens per user, with the desired refill rate and
interval.
...
We retrieve the characteristic, such as `userId`, and the user-provided prompt, then use them to withdraw the estimated number of tokens from the bucket.
