...
`UserPromptSubmit`. A wrong key denies every Codex and Cursor prompt and
...
Command entries in the managed layer (`requirements.toml` or the MDM
equivalent), not in a user `hooks.json`. No matcher, so every tool and
every prompt is covered. `PermissionRequest` is included because it is the
other `tool-call` event; it does not fire for calls that never ask for
approval, so it is not the control to rely on. `PreToolUse` is.

The command is one wrapper, invoked with the event name:
...
"PreToolUse": [
  {
    "hooks": [
      {
        "type": "command",
        "command": "/usr/local/bin/arcjet-hook codex pre-tool-use",
        "timeout": 5
      }
    ]
  }
...
"PermissionRequest": [
  {
    "hooks": [
      {
        "type": "command",
        "command": "/usr/local/bin/arcjet-hook codex permission-request",
        "timeout": 5
      }
    ]
  }
...
"UserPromptSubmit": [
  {
    "hooks": [
      {
        "type": "command",
        "command": "/usr/local/bin/arcjet-hook codex user-prompt-submit",
        "timeout": 5
      }
    ]
  }
...
Project hooks are not the install we document: they require trust review
and can be skipped.
