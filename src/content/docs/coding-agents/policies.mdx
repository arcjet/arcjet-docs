...
| `coding-agent.model-allowlist` | A model switch, prompt, or tool call whose model is outside the list in the policy. Claude Code denies the switch only; Codex and Cursor deny the prompt and the tool call; Copilot is not covered. |
...
`codingAgentMoments` is `["model-switch", "tool-call", "prompt"]`. All three
are required. A policy that reads `model` and is attached only to Tool call
refuses Codex and Cursor tools and never sees a Claude Code switch.

The set in the starter is a placeholder. The three ids are one per vendor we
enforce, so an author can see that the list is not Claude-only. Replace it
before the rule goes live. `auto` is not in the list, so a Cursor session on
Auto is denied. An author who intends to allow Auto adds `auto` to the list.
...
2. Set what it executes on: **Tool call** for anything about a command, a
   ...
   developer typed, and **Model switch** for a Claude Code model change. A
   model allowlist needs all three.
