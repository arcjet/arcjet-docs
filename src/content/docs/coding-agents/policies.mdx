...
| ---------------- | ----------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **Tool call**    | Before the agent runs a command, reads or writes a file, calls an MCP server, or fetches a URL  | `tool_name`, `tool_kind`, `command`, `command_tokens`, `paths`, `domains`, `destinations`, `mcp_server`, `mcp_tool`, `model` |
...
Write the allowlist into the policy. A list the agent supplied would be a list
...
it – use <Link.Page href="/guards/destination-threat">destination threat</Link.Page>
for that path.
...
Deny a tool call whose destination scores high or critical in Arcjet threat
intel – the same assessment Protect uses for the caller of an HTTP request.
Declare an Arcjet threat intel detector with the ID `dest` over `destinations`.

`destinations` holds the hosts of URL arguments plus the hosts of absolute URLs
inside a shell command. The rule does not require `tool_kind == "web"`, so a
`curl` in Bash is refused too.

Attach this to **Tool call**. `destinations` is empty on a prompt.
...
Name the set. A comparison with `>= "high"` is wrong: `critical` sorts before
`high`, and `none` sorts after it. An empty list is risk `none`. A lookup that
cannot be completed fails the policy closed (`AJP1009`).

If a reviewed host must **bypass** destination threat, do **not** publish
this starter beside `coding-agent.egress-allowlist`. Separate LIVE policies
take the most restrictive decision, so threat DENY still blocks an allowlisted
host. Write one custom policy instead: allowlist deny for unreviewed web hosts,
and threat deny only when the scored host is not on the allowlist. The shipped
starters cannot express that bypass on their own.
...
Declare detector ID `dest` over `destinations`, plus `tool_kind` and `domains`.
Publish both starters only when you want both constraints with no exception.
For signal fields, limits, and application Guard calls, see
...
- <Link.Page href="/guards/destination-threat">Destination threat detection</Link.Page> – Arcjet threat intel signals, allowlist bypass, and fail-closed behavior
