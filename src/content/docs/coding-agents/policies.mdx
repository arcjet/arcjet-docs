...
Publish both this policy and `coding-agent.egress-allowlist` on **Tool call**
when you want both controls. Arcjet runs every policy attached to that moment
and applies the most restrictive decision, so each policy still records its own
row and a host on the allowlist can still be denied when threat intel scores it
high or critical. For signal fields, limits, and application Guard calls, see
...
- <Link.Page href="/guards/destination-threat">Destination threat detection</Link.Page> – Arcjet threat intel signals, allowlists, and fail-closed behavior
