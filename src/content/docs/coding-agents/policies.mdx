...
Choose when the policy should execute: on a **Tool call**, on a **Prompt**, or
on a **Model switch**. A model list is a normal Rego policy with those
Execute on options, not a separate org-wide setting. The hook file does not
name it.
...
Events inside a moment still share inputs. `model` is filled only on the
vendors in [Where `model` is filled](#where-model-is-filled). A `model` rule
on Tool call or Prompt does nothing for Claude Code or Copilot, and that is
intended. It is not an INCOMPLETE policy.
...
we enforce, so an author can see that the list is not Claude-only. Replace
it with the ids you allow before the rule goes live. `auto` is not in the
list, so a Cursor session on Auto is denied unless the author listed
