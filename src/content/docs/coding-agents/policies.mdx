...
| ------------ | ---------------------------------------------------------- | --------------------------------------------- | ------------------------------------------ | ------------------------------- |
| `shell`      | `Bash`, `BashOutput`, `KillShell`                          | `powershell`                                  | `Bash`                                     | `Shell`                         |
| `file_read`  | `Read`, `Glob`, `Grep`, `NotebookRead`                     | `view`, `rg`                                  | Local function tools that read files       | `Read`, `Grep`                  |
| `file_write` | `Edit`, `Write`, `MultiEdit`, `NotebookEdit`               | `create`, `str_replace_editor`, `apply_patch` | `apply_patch` (also matches `Edit`, `Write`) | `Write`, `Delete`             |
| `web`        | `WebFetch`, `WebSearch`                                    | `web_fetch`, `web_search`                     | Hosted `WebSearch` skips `PreToolUse`      |                                 |
| `mcp`        | Any `mcp__<server>__<tool>` name                           | Any MCP tool                                  | Any `mcp__<server>__<tool>` name           | `MCP:<tool_name>`               |
| `agent`      | `Agent`, `Task`                                            |                                               | `Agent`, `spawn_agent`                     | `Task`                          |
...
2. Set what it executes on: **Tool call** for anything about a command, a
   file, a URL, or an MCP server, and **Prompt** for anything about what the
   developer typed.
