...
| `coding-agent.destination-threat` | A destination whose IP threat risk is high or critical. For demos, `evil.arcjet.com` and its subdomains always score high |
...
Deny a tool call whose destination scores high or critical in the same CrowdSec
and IPAPI database Protect uses for the caller of an HTTP request. Declare a
**server-side** IP threat detector with the ID `dest` over `destinations`.
...
For demos, `evil.arcjet.com` and its subdomains always score high without a DNS
or IP database lookup.

Publish this beside `coding-agent.egress-allowlist` when you want both an
allowlist of reviewed hosts and a refuse on high-risk destinations. For signal
fields, limits, and application Guard calls, see
...
- <Link.Page href="/guards/destination-threat">Destination threat detection</Link.Page> – IP threat signals, allowlists, and fail-closed behavior
