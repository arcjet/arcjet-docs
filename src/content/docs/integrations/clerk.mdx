---
title: "Arcjet and Clerk integration"
description: "Rate limit and screen requests by Clerk user or organization, so an Arcjet rule tracks the authenticated identity rather than the client IP address."
prev: false
next: false
---
...
<Link.Page href="/fingerprints#custom-characteristics">custom fingerprint characteristics</Link.Page>
to identify the client and apply the limit. Using
Clerk's [`currentUser()`](https://clerk.com/docs/references/nextjs/current-user)
(app router) or [`getAuth()`](https://clerk.com/docs/references/nextjs/get-auth)
...
If you want to protect every page with
<Link.Page href="/shield">Arcjet Shield</Link.Page> automatically you can run
it through Next.js middleware. Clerk also uses middleware to add authentication
to your pages. You can chain the two together.
