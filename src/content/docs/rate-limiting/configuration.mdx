...
a `string` with a sequence of numbers, each with a unit suffix, such as `1s` for
1 second, `1h45m` for 1 hour and 45 minutes, `1d` for 1 day.
...
set the interval to `60` the sliding window is 60 seconds.

For `tokenBucket`, the time interval for the refill rate (described later on
this page). For example, if you set the interval to `60` and the refill rate to
`10`, the bucket refills 10 tokens every 60 seconds.
...
numbers, each with a unit suffix, such as `1s` for 1 second, `1h45m` for 1 hour
and 45 minutes, `1d` for 1 day.
...
The number of tokens to add to the bucket at each interval (described earlier on
this page). This is an integer value ranging from `0` to `4,294,967,295`. For
example, if you set the interval to `60` and the refill rate to `10`, the bucket
refills 10 tokens every 60 seconds.
...
The maximum number of tokens the bucket can hold. The bucket refills until it
hits the capacity. This is an integer value ranging from `0` to `4,294,967,295`.
