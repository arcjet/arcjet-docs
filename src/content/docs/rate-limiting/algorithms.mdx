...
This algorithm tracks the number of requests made by a client over a fixed time
window, such as 60 seconds. If the client exceeds the
limit, they are blocked until the window expires.

### When to use fixed window

This algorithm is useful when you want to apply a fixed limit in a fixed time
window. For example, a flat limit on the total number of requests a client can
make.

### Downsides of fixed window

- A burst of requests at the start of the window exhausts the limit for newer
  requests.
...
Suppose the time is `12:00:00` and a client makes a burst of 50 requests, then
another 50 requests over the following 30 seconds. Arcjet blocks any further
requests until `12:01:00`. At `12:01:00` the window resets and they could make
another 100 requests immediately.
...
Suppose the client made 50 requests between `12:00:00` and `12:01:00`, then made
another 50 requests in the current time period as of `12:01:30`. Arcjet
calculates the remaining rate limit as follows:
...
A burst of requests can exhaust the bucket, which then takes a long time to
refill, depending on your configured refill rate. This can block a client for a
long time.
...
option when calling the Arcjet `protect` function. You can request any number of
tokens up to the capacity of the bucket.
...
```py
# Deduct 1 token from the bucket
decision = await aj.protect(request, requested=1)  # use aj.protect(request, ...) for arcjet_sync
```
</TabItem>
</Tabs>

For example, if it is `12:00:00` and a client makes 100 requests each with 1
token deducted, further requests are blocked until `12:01:00`, when the bucket
refills by 10 tokens. The client could then make 10 requests over the following
60 seconds. If they make no requests, the bucket refills to 20 tokens at
`12:02:00`. If they make no requests between `12:01:00` and `12:10:00` then the
bucket refills to 100 tokens at `12:10:00`. No further tokens are added after it
reaches 100 tokens.
