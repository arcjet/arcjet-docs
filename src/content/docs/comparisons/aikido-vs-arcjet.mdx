...
1. **Security components for developers**: Arcjet provides a range of security
   ...
   budget enforcement, and PII detection in AI request/response flows. Aikido's
...
2. **Integration**: Arcjet is designed to integrate as a library directly into
   ...
   works with any modern JS/TS framework or bundler, and any Python application
   (supporting both async and sync APIs) and rules can be adjusted dynamically
   at runtime as regular code. Aikido's Zen Firewall for JS only works with
...
3. **Framework, runtime & language support:** Arcjet currently focuses on the
   ...
   PHP, Java, .NET, and Ruby. However, Aikido's monkey-patching approach causes
   issues with certain frameworks, such as [not being able to use custom rate
   limits by user ID in
...
5. **Advanced client signals for bot detection:** Arcjet provides
   ...
   a browser-based detection layer that runs a WebAssembly module in the
   client to collect environment signals and identify headless browsers and
   sophisticated scrapers — without CAPTCHAs or user challenges. Aikido's bot detection remains
   focused on user-agent signature matching and IP-based controls, with no
   equivalent to client-side signal collection or expression-based filter rules.

## Comparison

| Area                     | Arcjet                                                                                                                                                                                                                                                    | Aikido (Zen Firewall)                                                                                                                                                                                                            |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Product scope            | In-code security SDK for Javascript, TypeScript, and Python apps: bot protection, rate limiting, WAF (Shield), email validation, signup protection, sensitive info detection & redaction, filter rules, and AI Runtime Protection (prompt injection detection, per-user token budgets, PII in AI flows). | Firewall focused on Node.js for JavaScript, with agents for other languages. No dedicated AI runtime protection features.                                                                                                        |
...
- **You care about strong primitives beyond "firewall"** Bot protection, rate
  ...
  filter rules, and AI Runtime Protection are all part of the Arcjet product
  suite. You'd rather compose these into flows than sprinkle generic "block this
  request" calls around.
...
- **You're already bought into Aikido's platform** You want Zen as another
  ...
  controls, and are happy with CommonJS-only Node.js-only monkey-patching.
