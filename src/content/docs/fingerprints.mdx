...
for logged in users. Use `withRule` to create augmented clients that use
...
<TabItem label="Python">
```py
# The Python SDK does not have `withRule`. Define a separate client per rule
# set (e.g. `aj_for_users` and `aj_for_guests`) at module scope, then pick the
# one to use inside each request handler.
